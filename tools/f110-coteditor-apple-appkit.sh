#!/usr/bin/env bash
# f110-coteditor-apple-appkit.sh [label] — F107 step 3 on a REAL GUI app: the F104 CotEditor
...
A=$W/artifacts/f110-coteditor
out=$W/f110-coteditor/$LABEL; mkdir -p "$out"
...
sed -E 's|--name "\$container" --rm|--name "$container" --rm --ulimit core=-1|' \
	tools/probe-iterm2-launch-arm64.sh > tools/f110-tmp-probe.sh && chmod +x tools/f110-tmp-probe.sh
grep -q 'ulimit core' tools/f110-tmp-probe.sh || { echo "probe sed failed"; exit 1; }

echo "F110: Stage 20 / CotEditor 7.0.7 — A/B/A over PREFER_DISK_FRAMEWORKS, label=$LABEL"
...
for spec in "${ARMS[@]}"; do
	...
	echo "[arm $i] PREFER_DISK_FRAMEWORKS=$prefer $(date +%H:%M:%S)"
	env \
		...
		tools/f110-tmp-probe.sh > "$out/$i-probe.log" 2>&1
	rc=$?
	k="$out/$i"; mkdir -p "$k"
	...
	echo "  first failure line: $(grep -m1 -E "Symbol not found|Library not loaded|Terminating app|uncaught exception|abort|Trace/BPT|Segmentation|Assertion" "$k/iterm2-job.err" 2>/dev/null || echo "<none>")"
...
restore; trap - EXIT
rm -f tools/f110-tmp-probe.sh
echo "F110 COMPLETE $(date +%H:%M:%S) — artifacts in $out"
